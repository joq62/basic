...
test_para_server()->
    ...
    P_Server= misc_lib:get_node_by_id(?P_SERVER),
    {I1,P1}=?P_IP_OK,
 %   ok=rcp:call(P_Server,lib_service,stop_tcp_server,[{I1,P1}]),
    os:cmd("rm -rf "++?P_SERVER),
    rpc:call(P_Server,init,stop,[]),
    timer:sleep(2000),
    ...
    timer:sleep(100),
    {I1,P1}=?P_IP_OK,
    ?assertEqual(ok,rpc:call(P_Server,lib_service,start_tcp_server,[I1,P1,parallell])),
    
    D=date(),
    ?assertEqual(D,tcp_client:call(?P_IP_OK,{erlang,date,[]},5000)),
    ?assertMatch({error,[nxdomain,"joqhome.dynamic-dns.glurk",40000,
			tcp_client,_]},tcp_client:call(?P_WRONG_IP,{erlang,date,[]},10000)),
    ?assertMatch({error,[timeout,"joqhome.dynamic-dns.net",22000,
			 tcp_client,_]},tcp_client:call(?P_WRONG_PORT,{erlang,date,[]},5000)),

    {ok,Socket}=tcp_client:connect(I1,P1,5000),
    ok=tcp_client:cast(Socket,{erlang,date,[]}),
    D=tcp_client:get_msg(Socket,2000),
    


    %%---- Delete pod -------------------------------------------------------
    D=date(),
    ?assertEqual(D,rpc:call(P_Server,erlang,date,[])),
    ?assertEqual({ok,stopped},pod:delete(?P_SERVER)),
    ?assertEqual({badrpc,nodedown},rpc:call(P_Server,erlang,date,[])),    
    ok.
